...
import {
  ...
  CheckCircle,
  XCircle,
  MapPin, // <--- Nuevo ícono importado
} from "lucide-react";

// 👇 Imports para el mapa
import { LabsMap } from "../components/maps/LabsMap";
import { adminService } from "../services/admin.service";

const TuberculosisInfo = () => {
  // 👇 Estado para los laboratorios
  ...
  useEffect(() => {
    const fetchLabs = async () => {
      try {
        const data = await adminService.getLaboratorios();
        setLabs(data);
      } catch (error) {
  ...
  const symptoms = [
    {
      ...
      description: "Tos por más de 2 semanas, a veces con sangre",
    },
    {
      icon: Activity,
      title: "Fiebre",
      description: "Fiebre persistente, especialmente nocturna",
    },
    {
      icon: Droplet,
      title: "Sudoración",
      description: "Sudores nocturnos abundantes",
    },
    {
      icon: TrendingDown,
      title: "Pérdida de Peso",
      description: "Pérdida de peso no intencional",
    },
  ];
    ...
    <div className="max-w-7xl mx-auto space-y-8 px-4 sm:px-6 pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-gradient-to-br from-green-500/10 to-green-600/10 border border-green-500/20 rounded-2xl p-6 hover:shadow-xl hover:shadow-green-500/10 transition-all duration-300">
          ...
          <p className="text-gray-300 text-sm">
            tasa de curación con tratamiento adecuado y completo
          </p>
        </div>
        ...
        <div className="bg-gradient-to-br from-blue-500/10 to-blue-600/10 border border-blue-500/20 rounded-2xl p-6 hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300">
          ...
          <p className="text-2xl sm:text-3xl font-bold text-blue-400 mb-2">
            Vacuna BCG
          </p>
          <p className="text-gray-300 text-sm">
            protege contra formas graves en niños
          </p>
        </div>
      </div>
        ...
        <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-2xl p-4 sm:p-6 hover:border-blue-500/30 transition-all duration-300">
          ...
          <div className="space-y-3">
            {symptoms.map((symptom, index) => {
              ...
              return (
                <div
                  key={index}
                  className="flex items-start gap-3 sm:gap-4 p-3 sm:p-4 bg-gray-700/30 rounded-xl hover:bg-gray-700/50 transition-all duration-200"
                >
          ...
          <div className="mt-6 p-3 sm:p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-xl">
            <p className="text-yellow-200 text-xs sm:text-sm">
              <strong>Importante:</strong> Si presentas tos por más de 2
              semanas, consulta inmediatamente con un profesional de salud.
            </p>
          </div>
        </div>
            ...
            <div className="space-y-3">
              {prevention.map((item, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-green-500/5 rounded-xl border border-green-500/10"
                  >
        ...
        <div className="flex flex-col sm:flex-row sm:items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-300 text-sm sm:text-base mb-4 leading-relaxed">
              ...
              aire. Cuando una persona con TBC pulmonar tose, estornuda o habla,
              expulsa bacilos de tuberculosis al aire. Basta con inhalar algunos
              de estos bacilos para quedar infectado.
            </p>
      ...
      <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-2xl p-4 sm:p-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
          <div className="text-center">
            <div className="bg-blue-500/10 w-12 h-12 sm:w-16 sm:h-16 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
              <span className="text-xl sm:text-3xl font-bold text-blue-400">
                6
              </span>
            </div>
          ...
          <div className="text-center">
            ...
            <p className="text-gray-400 text-xs sm:text-sm">
              El tratamiento es gratuito en todo el país
            </p>
          </div>
          <div className="text-center">
            ...
            <p className="text-gray-400 text-xs sm:text-sm">
              Alta tasa de curación si se completa
            </p>
          </div>
        </div>
        <div className="mt-6 p-3 sm:p-4 bg-red-500/10 border border-red-500/20 rounded-xl">
          <p className="text-red-200 text-xs sm:text-sm text-center">
            <strong>Crucial:</strong> NO abandonar el tratamiento. Suspenderlo
            puede generar resistencia a los medicamentos y complicar la
            curación.
          </p>
        </div>
      </div>

      {/* 👇 SECCIÓN NUEVA: MAPA DE LABORATORIOS */}
      <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-2xl p-4 sm:p-8">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-6">
          <div className="bg-blue-500/10 p-3 rounded-xl w-fit">
        ...
        <p className="text-gray-300 mb-6 text-sm sm:text-base">
          Encuentra el laboratorio o centro de salud más cercano para realizar
          tu diagnóstico. Haz clic en los marcadores para ver más detalles.
        </p>
