...
import { STEP_REGISTRY, UnknownStep } from './WizardRegistry';
...
const WizardContainer: React.FC = () => {
  ...
  const handleNext = (payload?: number | string) => {
    ...
    switch (stepData.code) {
      case 'STEP_PROVINCE':
        setContext(prev => ({ ...prev, selectedProvinceId: Number(payload) }));
        setCurrentStepId(2);
        break;
    ...
    return (
      <div className="flex justify-center items-center min-h-[400px] p-4">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-gray-300 text-sm sm:text-base">Cargando asistente...</p>
        </div>
      </div>
    );
  }
  ...
  if (!stepData) return (
    <div className="flex justify-center items-center min-h-[400px] p-4">
      <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-6 max-w-md w-full">
        ...
        <p className="text-gray-300 text-sm">No se pudo cargar la información. Por favor, intenta nuevamente.</p>
        <button 
  ...
  return (
    <div className={`flex justify-center w-full px-4 transition-all duration-300 ${
        ...
        {transitioning && (
          <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-gray-300 text-sm">Cargando siguiente paso...</p>
            </div>
          </div>
        )}
      </div>
